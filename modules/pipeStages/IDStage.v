...
module IDStage (clk, rst, hazard_detected_in, is_imm_out, ST_or_BNE_out, instruction, reg1, reg2, src1, src2_reg_file, src2_forw, val1, val2, brTaken, EXE_CMD, MEM_R_EN, MEM_W_EN, WB_EN, branch_comm);
  ...
  wire CU2and, CondTrue;
  wire [1:0] CU2Cond;
  wire Is_Imm, ST_or_BNE;
  wire [`WORD_LEN-1:0] signExt2Mux;

  assign brTaken = CU2and && CondTrue;
  assign val1 = reg1;
  ...
  mux #(.LENGTH(`REG_FILE_ADDR_LEN)) mux_src2 ( // determines the register source 2 for register file
  ...
  mux #(.LENGTH(`WORD_LEN)) mux_val2 ( // determines whether val2 is from the reg file or the immediate value
    .in1(reg2),
    .in2(signExt2Mux),
    .sel(Is_Imm),
    .out(val2)
